...
import {api, apiUrl, ApiError, consumeEvents, responseError, runErrorMessage, type Attachment, type Conversation, type Message, type Run, type Source, type Usage, type User} from '@/lib/api';
...
function DeleteAccount({disabled,onDelete}:{disabled:boolean;onDelete:()=>Promise<void>}) {
  const [pending,setPending]=useState(false);
  const [error,setError]=useState('');
  return <AlertDialog.Root><AlertDialog.Trigger disabled={disabled} className="danger">계정 삭제</AlertDialog.Trigger><AlertDialog.Portal><AlertDialog.Backdrop className="backdrop"/><AlertDialog.Popup className="popup"><AlertDialog.Title className="popup-title">계정을 삭제할까요?</AlertDialog.Title><AlertDialog.Description className="popup-description">모든 대화와 첨부파일을 삭제하고 로그아웃합니다. 개인 정보는 지우고 비용 기록만 남깁니다. 되돌릴 수 없습니다.</AlertDialog.Description>{error&&<p role="alert" className="error">{error}</p>}<div className="popup-actions"><AlertDialog.Close disabled={pending}>취소</AlertDialog.Close><button className="danger" disabled={pending} onClick={async()=>{setPending(true);try{await onDelete()}catch{setError('계정을 삭제하지 못했습니다. 진행 중인 답변을 중단한 뒤 다시 시도해 주세요.')}finally{setPending(false)}}}>{pending?'삭제 중':'계정과 대화 삭제'}</button></div></AlertDialog.Popup></AlertDialog.Portal></AlertDialog.Root>
}
...
export default function Home() {
  ...
  useEffect(()=>{
    ...
    if(!initialLoad.current){
      ...
      void (async()=>{
        try{
          const [configuration,session]=await Promise.allSettled([api<Config>('/v1/config'),api<User>('/v1/me')]);
    ...
    <main className="workspace">
      ...
      <div className="composer-zone"><div className="reading">
        ...
        <form className="composer" onSubmit={e=>{e.preventDefault();send()}}>
          ...
          <label htmlFor="question" className="sr-only">질문 입력</label><textarea aria-describedby={error?'composer-error':undefined} id="question" ref={input} value={draft} onChange={e=>setDraft(e.target.value)} placeholder={user?'질문을 입력하거나 자료를 첨부하세요.':'로그인 후 질문을 입력하세요.'} disabled={!user||busy||opening} maxLength={12000} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey&&!e.nativeEvent.isComposing&&window.matchMedia('(hover: hover) and (pointer: fine)').matches){e.preventDefault();send()}}}/>
