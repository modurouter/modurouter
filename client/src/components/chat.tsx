...
export function Chat() {
  ...
  const attachments = useAttachments(chatApi.prepareConversation);
  const composer = useRef<HTMLFormElement>(null);
  const draggingFile = useFileDrop(composer, files => { void attachments.add(files); });
  ...
  }, [pendingVoice, streaming, attachments.ready]);
  useEffect(() => {
    const onScroll = () => {
  ...
  function send(retryText?: string, retryFiles?: Attachment[], clearDraft = retryText === undefined, context = learningContext?.mode === mode ? learningContext.prompt : '') {
    ...
    if (!text || useChatStore.getState().streaming || (clearDraft && retryFiles === undefined && attachments.blocked)) return;
    ...
    void chatApi.send(text, mode, retryFiles ?? attachments.ready, context);
  ...
  return <MotionConfig reducedMotion="user"><main ref={root} className={`app mode-${mode} ${messages.length ? 'has-messages' : ''}`}>
    <LearningHeader mode={mode} progress={learning.progress[mode]} ready={learning.ready} onPracticeChoice={index => { setRequestError(''); learning.choose(mode, index); }} onRestart={() => learning.restart(mode)} onClear={() => learning.clear(mode)} view={learningView} onViewChange={view => { if (view.practice) setLearningContext(null); setLearningView(view); }} disabled={streaming || speech.recording || speech.processing} onChoose={(prompt, label) => { setLearningContext({ mode, prompt, label }); send(label, attachments.ready, false, prompt); }} />
    <section className="workspace" aria-label="대화">
      ...
      {messages.length > 0 && <div className="conversation" role="log" aria-label="대화 내용" aria-live="off">
        {messages.map((message, messageIndex) => <motion.article initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} key={message.id} className={`message message-${message.role}`} aria-label={message.role === 'user' ? '내 질문' : '답변'}>
          ...
          {message.role === 'assistant' && message.activity?.status !== 'writing' && <div className="answer-actions">
            ...
            <Button disabled={streaming} onClick={() => { const question = messages.slice(0, messageIndex).findLast((item) => item.role === 'user'); if (question) send(question.content, question.attachments); }}><RotateCcw size={15} /><span>다시 생성</span></Button>
      ...
      <div className="controls" ref={controls}>
        ...
        <div className="input-row">
          <form ref={composer} className={`composer glass ${draggingFile ? 'is-file-over' : ''}`} onPaste={e => { if (!streaming && e.clipboardData.files.length) { e.preventDefault(); void attachments.add(Array.from(e.clipboardData.files)); } }} onSubmit={(e) => { e.preventDefault(); send(); }}>
            <GlassSurface />
            {!!attachments.selected.length && <div className="composer-attachments">{attachments.selected.map(file => <div key={file.localId}><AttachmentCard file={file} onRemove={() => attachments.toggle(file.localId)} />{file.status === 'delayed' && <button type="button" className="attachment-refresh" onClick={() => void attachments.refresh(file)}>다시 확인</button>}</div>)}</div>}
            ...
            <AttachmentMenu attachments={attachments} disabled={streaming} />
            <textarea ref={textarea} aria-label="메시지 입력" placeholder={currentMode.placeholder} value={input} rows={1} maxLength={12000} onChange={(e) => { setPendingVoice(null); setInput(e.target.value); }} />
            <motion.button whileTap={{ scale: .92 }} className={`send-button ${input.trim() || streaming ? 'is-ready' : ''}`} type={streaming ? 'button' : 'submit'} onClick={streaming ? () => stop() : undefined} disabled={!streaming && (!input.trim() || attachments.blocked)} aria-label={streaming ? '응답 중지' : '메시지 전송'}>{streaming ? <Square size={17} fill="currentColor" /> : <ArrowUp size={29} strokeWidth={1.7} />}</motion.button>
            </div>
          </form>
          <motion.button className={`microphone glass ${recording ? 'is-recording' : ''}`} whileHover={{ scale: 1.045 }} whileTap={{ scale: .94 }} disabled={speech.processing} onClick={() => { if (recording) speech.stop(); else { setPendingVoice(null); setRequestError(''); void speech.start(input); } }} aria-label={recording ? '녹음 종료 후 전송' : '음성 입력'} title="음성으로 입력하기 · 최대 10분" aria-pressed={recording}><GlassSurface radius={50} tone="accent" />{recording ? <Square size={21} strokeWidth={1.5} /> : <Mic size={27} strokeWidth={1.45} />}</motion.button>
        </div>
