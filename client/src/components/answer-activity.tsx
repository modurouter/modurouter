...
export function AnswerActivity({ message }: { message: Message }) {
  ...
  return <Collapsible.Root className="answer-activity">
    <Collapsible.Trigger className="activity-trigger">
      <span>{label}</span>{(message.run?.selected_model || message.selectedModel) && <span className="active-model">{message.run?.selected_provider || message.selectedProvider} / {message.run?.selected_model || message.selectedModel}</span>}<ChevronRight size={14} className="activity-chevron" />
    </Collapsible.Trigger>
    <Collapsible.Panel className="activity-panel">
      {activity.events?.map((event, i) => <p key={i}>{event}</p>)}
      {(message.run?.selected_model || message.selectedModel) && <p>사용 모델: {message.run?.selected_model || message.selectedModel}</p>}
