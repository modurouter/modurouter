...
function ConfirmDelete({label, description, action, disabled, showLabel = false}: {label:string; description:string; action:()=>Promise<void>; disabled:boolean; showLabel?:boolean}) {
  ...
  return <AlertDialog.Root open={open} onOpenChange={value=>{if(!busy)setOpen(value);}}>
    ...
    <AlertDialog.Portal><AlertDialog.Backdrop className="confirm-backdrop"/><AlertDialog.Popup className="confirm-popup liquid-popup">
      <GlassSurface radius={28} /><div className="liquid-popup-content confirm-content">
    ...
    </div></AlertDialog.Popup></AlertDialog.Portal>
  </AlertDialog.Root>;
}
export function WorkspaceMenu({workspace,disabled,onNavigate}: {workspace:ReturnType<typeof useChatWorkspace>; disabled:boolean; onNavigate:()=>void}) {
  ...
  return <nav className="workspace-nav" aria-label="대화 관리">
    ...
    <Popover.Root open={open} onOpenChange={setOpen}>
      ...
      <Popover.Portal><Popover.Positioner side="bottom" align="end" sideOffset={8} collisionPadding={16} className="workspace-positioner"><Popover.Popup className="workspace-popup liquid-popup">
        <GlassSurface radius={28} /><div className="liquid-popup-content workspace-popup-content">
