...
export function useChatApi() {
  ...
  useEffect(() => {
    const leave = () => {
      if (current.current?.runId && session.current) void fetch(apiUrl(`/v1/runs/${current.current.runId}/cancel`), { method: 'POST', credentials: 'same-origin', keepalive: true, headers: { 'X-CSRF-Token': session.current.csrf_token } }).catch(() => {});
    };
  ...
  async function send(text: string, mode: Mode) {
    ...
    try {
      ...
      if (error instanceof ApiError && error.status === 401) { session.current = null; conversation.current = null; uncertain.current = null; }
