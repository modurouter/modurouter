...
Stopping the recording transcribes the audio and automatically submits the result to chat, including the draft captured when recording started and ready attachments. If submission is blocked or the text exceeds the chat limit, the transcript remains in the composer. Empty/failed transcription and cancellation do not submit a chat message. Closing the voice panel cancels a pending recording/upload. Audio is not saved to disk or the database. Browser permission denial, unsupported recording, missing server setup, provider failure and empty transcripts receive UI messages.
...
- Apply migration `0006` using the existing release process (`uv run alembic upgrade head` from `server/`).
...
- Production release status is recorded in `deployment-2026-09-18-models-speech.md`. No paid transcription was performed during verification.
...
Each STT request consumes one existing daily request allowance and participates in the user and shared guest dollar budget. A conservative $0.03 hold is reserved before the call, then replaced with the cost calculated from returned usage. This hold is not the expected price. An ambiguous network interruption or missing usage keeps the reservation pending; it is never silently treated as free or automatically resent. `speech_requests` records reservation status without storing audio or transcript. Pending STT holds require operator review; the existing chat generation reconciliation worker does not reconcile them. Above-reservation cost blocks further paid requests as it does for chat.
