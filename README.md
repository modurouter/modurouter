...
웹 검색 자동 모드에서는 검색 요청이나 최신 정보가 필요한 표현을 감지합니다. 첨부파일이 있는 질문과 그 후속 질문은 명시적인 검색 요청이 있을 때 검색하며 질문에 쓴 URL은 검색 모드와 관계없이 먼저 읽습니다. DuckDuckGo 검색 결과는 최대 5개를 수집하고 그중 최대 4개의 원문을 추가로 확인합니다. 다만 실행 시간과 도구 호출 한도 안에서 처리하므로 모든 검색 결과를 읽는 것은 아닙니다. 검색어로는 현재 질문의 앞 500자만 전송하며 첨부파일에서 추출한 본문을 덧붙이지 않습니다.
...
생활 배움터와 호기심 교실, 학습 스튜디오에서 단계별 실습을 진행합니다. 이어 하기와 완료 기록은 현재 브라우저에 저장됩니다. 상단에는 공식 로고가 흐르며 입력창의 + 메뉴에서 파일과 사진을 첨부할 수 있습니다. 붙여넣기와 드래그 앤 드롭도 지원합니다. 자세한 범위는 [모드별 학습 공간](docs/mode-learning.md)을 참고하세요.
